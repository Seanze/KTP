<template>
  <div class="course">
    ...
    <div class="header-info" style="margin-top: -30px">
      <p class="time" style="float: left"> {{semester}}</p>
      <p class="time" >{{ term === 1 ? "第一学期" : "第二学期" }}</p>
      <router-link :to="'detail/'+cid" class="name">{{title}}</router-link>
      <h3 style="color: white;font-size: 16px">{{className}}</h3>
      <div >
        <span style="font-size: 14px;color: white">课堂码：{{inviteCode}}</span>
      </div>
    </div>
    ...
    <el-row type="flex" align="middle" style="margin: 5px 18px">
      ...
      <el-col :span="4">
        <div style=" margin-left: 10px;font-size: 12px;font-weight: 500;color: rgba(95, 99, 104, 1);cursor: default">{{
            teacherName
          }}
        </div>
      </el-col>
